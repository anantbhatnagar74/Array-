...
public class FourSum 
{
	...
	// Function to find the all the unique quadruplets with the elements at different indices
	public static void GetQuadruplets(ArrayList<Integer> nums,int target)
	{
		// Stores the sum mapped to a List Of Pair<i, j>
		HashMap<Integer, ArrayList<Pair> > map = new HashMap<>();
		// Generate all possible pairs for the HashMap
		for (int i = 0;
			i < nums.size() - 1; i++) {
			for (int j = i + 1; j < nums.size(); j++) 
			{
				...
				// If the sum doesn't exists then update with the new pairs
				...
				// Otherwise, push the new pair of indices to the current sum
				else 
				{
